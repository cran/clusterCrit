...
the datat matrix $A$. Similarly, let us denote by $G$ the barycenter of these
...
a_{j}$, one says that the two vectors are concordant if $b_{i} <
...
$\{i,j\}$ and the number $s^{-}$ of discordant pairs.

The $\Gamma$ index is classically defined like this:
\begin{equation}
...
$\{M_{i},M_{j}\}$ has value 1 if the two points lie in the same
cluster and 0 otherwise. These two vectors have length
$N_{T}=N(N-1)/2$.

The number $s^{+}$ represents the number of times a distance between
two points which do not belong to the same cluster (that is to say a
pair for which the value of vector $B$ is 0) is strictly smaller than
the distance between two points belonging to the same cluster (that is
...
that is to say that a distance between two points which do not belong
to the same cluster (value 0 in $B$) is strictly greater than a
distance between two points lying in the same cluster (value 1 in
$B$). The cases where there is equality (ties or \emph{ex-aequos}) are
not taken into account. There are $N_{B}$ between-cluster distances
and, for each of them, one compares with the $N_{W}$ within-cluster
distances: one finally performs $N_{B}\times N_{W}$ comparisons.

One can write the numbers $s^{+}$ and $s^{-}$ in the following form:
\begin{eqnarray}
    s^{+} & = & \sum_{(r,s)\in I_{B}}\sum_{(u,v)\in I_{W}} 
    \mathbf{1}_{\{d_{rs} < d_{uv}\}} \\
    s^{-} & = & \sum_{(r,s)\in I_{B}}\sum_{(u,v)\in I_{W}} 
    \mathbf{1}_{\{d_{rs} > d_{uv}\}}
\end{eqnarray}
...
Their difference is:
\begin{equation}
s^{+}-s^{-}=\sum_{(r,s)\in I_{B}}\sum_{(u,v)\in I_{W}} \sgn(d_{uv} - d_{rs})
\end{equation}
